...
var (
	...
	ErrGrabPublisher = errors.New("发布者不能参与自己的拼单") // 契约：发布者不可抢自己的单
	ErrGrabBusy      = errors.New("抢单繁忙，请稍后重试")   // 锁竞争失败（≠售罄）
)

// GrabGroupBuy 抢单（POST /api/v1/group-buy/:id/grab，契约 mvp §4.3 生产者侧六步）。
// userID 来自强制 JWT，goodID 来自路径参数。
//
// 六步编排（每步的失败策略各不相同——分层失败策略 mvp §9 的集中体现）：
...
func GrabGroupBuy(userID, goodID int64) (*model.GrabResult, error) {
	...
	// 宁可少卖（可对账修复），不可超卖（不可逆）。丢失的兜底：阶段7 producer confirm +
	// error 日志人工对账。
...
// 极端场景（预扣成功后拼单翻终态）由阶段6 延时关单流程处理，不在本接口职责内。
